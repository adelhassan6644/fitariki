...
class PaymentProvider extends ChangeNotifier {
  ...
  calcTotal() {
    ...
    total = 0;
    tax = double.parse(
        (((requestModel?.price ?? 0)-discount) * taxPercentage / 100).toStringAsFixed(2));
    serviceCost = double.parse(
        (((requestModel?.price ?? 0)-discount) * servicePercentage / 100)
            .toStringAsFixed(2));

    if ((requestModel!.price! + tax! + serviceCost - discount) <= wallet) {
      walletDeduction = (requestModel!.price! + tax! + serviceCost - discount);
    } else {
